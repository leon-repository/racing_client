...
namespace WxGames.HTTP
{
    ...
    public class PerFormerService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public JObject WxInit(Token token)
        {
            ...
            Cookie sid = service.GetCookie("wxsid", token.Container);

            Cookie uin = service.GetCookie("wxuin", token.Container);

            if (sid != null && uin != null)
            {
                init_json = string.Format(init_json, uin.Value, sid.Value, token.SKey, s.ToString());
                ...
                {
                    HttpService service2 = new HttpService();
                    string url = "https://" + item + "/cgi-bin/mmwebwx-bin/webwxinit?";
                    bytes = service2.SendPostRequest(url + "r=" + r + "&lang=zh_CN&pass_ticket=" + token.Pass_Ticket, init_json);
                    if (bytes!= null)
                    {
        ...
        /// </summary>
        /// <returns></returns>
        public JObject GetContact(Token token)
        {
            HttpService service = new HttpService();

            List<string> listCheckUrl = new List<string>();
            listCheckUrl.Add("https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxgetcontact");
            listCheckUrl.Add("https://wx2.qq.com/cgi-bin/mmwebwx-bin/webwxgetcontact");
            string contact_str = "";
            foreach (string item in listCheckUrl)
            {
                string r = DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalMilliseconds.ToString().Split(new char[] { '.' })[0];

                byte[] bytes = service.SendGetRequest(item + "?lang=zh_CN&pass_ticket="+token.Pass_Ticket+"&r=" + r + "&seq=0&skey=" + token.SKey, token.Container);
                if (bytes == null || bytes.Length == 0)
                {
                    return null;
                }
                contact_str = Encoding.UTF8.GetString(bytes);
                if (contact_str.Contains("\"Ret\": 0"))
                {
                    break;
                }
            }
