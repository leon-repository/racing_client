...
namespace WxGames.HTTP
{
    ...
    public class PerformerLogin
    {
        ...
        //获取会话ID的URL
        private string _session_id_url = "https://login.weixin.qq.com/jslogin?appid=wx782c26e4c19acffb";
        //获取二维码的URL
        ...
        {
            byte[] bytes = service.SendGetRequest(_session_id_url,new CookieContainer());
            _session_id = Encoding.UTF8.GetString(bytes).Split(new string[] { "\"" }, StringSplitOptions.None)[1];
            bytes = service.SendGetRequest(_qrcode_url + _session_id, new CookieContainer());
        ...
        public object LoginCheck()
        {
            ...
            byte[] bytes = service.SendGetRequest(_login_check_url + _session_id,new CookieContainer());
        ...
        /// </summary>
        public Token GetSidUid(string login_redirect)
        {

            byte[] bytes = service.SendGetRequest(login_redirect + "&fun=new&version=v2&lang=zh_CN",new CookieContainer());
            ...
            //获取Cookie，获取Uin
            List<Cookie> cookList=service.GetAllCookies(service.CookiesContainer);

            Token token = new Token();
            token.Pass_Ticket = Pass_Ticket;
            token.SKey = SKey;
            token.CookList = cookList;
            token.Container = service.CookiesContainer;
