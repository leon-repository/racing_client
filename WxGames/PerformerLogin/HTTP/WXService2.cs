...
namespace WxGames.HTTP
{
    ...
    public class WXService2
    {
        ...
        public List<Cookie> GetCookList()
        {
            return BaseService2.GetAllCookies(BaseService2.CookiesContainer);
        }
        ...
        public JObject WxInit()
        {
            ...
            Cookie sid = BaseService2.GetCookie("wxsid");
            Cookie uin = BaseService2.GetCookie("wxuin");

            if (sid != null && uin != null)
            {
                init_json = string.Format(init_json, uin.Value, sid.Value,LoginService2.SKey,s.ToString());
                ...
                foreach (string item in listCheckUrl)
                {
                    ...
                    bytes = BaseService2.SendPostRequest(url + "r=" + r + "&lang=zh_CN&pass_ticket=" + LoginService2.Pass_Ticket, init_json);
                    if (bytes!= null)
                    {
        ...
        {
            byte[] bytes = BaseService2.SendGetRequest(_geticon_url + username);
        ...
        {
            byte[] bytes = BaseService2.SendGetRequest(_getheadimg_url + usename);
            if (bytes == null || bytes.Length == 0)
            {
                return null;
            }
        ...
        {
            byte[] bytes = BaseService2.SendGetRequest(_getcontact_url);
            if (bytes == null || bytes.Length == 0)
            {
                return null;
            }
        ...
        public string WxSyncCheck()
        {
            ...
            if (sid != null && uin != null)
            {
                ...
                foreach (var item in listCheckUrl)
                {
                    ...
                    _synccheck_url = string.Format(url, sid.Value, uin.Value, sync_key, (long)(DateTime.Now.ToUniversalTime() - new System.DateTime(1970, 1, 1)).TotalMilliseconds, LoginService2.SKey.Replace("@", "%40"), "e1615250492");
                    bytes = BaseService2.SendGetRequest(_synccheck_url + "&_=" + DateTime.Now.Ticks);
                    if (bytes != null)
                    {
        ...
        public JObject WxSync()
        {
            ...
            sync_json = string.Format(sync_json, uin.Value, sid.Value, _syncKey.Count, sync_keys, (long)(DateTime.Now.ToUniversalTime() - new System.DateTime(1970, 1, 1)).TotalMilliseconds, LoginService2.SKey);

            if (sid != null && uin != null)
            {
                byte[] bytes = BaseService2.SendPostRequest(_sync_url + sid.Value + "&lang=zh_CN&skey=" + LoginService2.SKey + "&pass_ticket=" + LoginService2.Pass_Ticket, sync_json);
        ...
        public void SendMsg(string msg, string from, string to, int type)
        {
            ...
            if (sid != null && uin != null)
            {
                msg_json = string.Format(msg_json, sid.Value, uin.Value, msg, from, to, type, LoginService2.SKey, DateTime.Now.Millisecond, DateTime.Now.Millisecond, DateTime.Now.Millisecond);
                ...
                foreach (string item in listCheckUrl)
                {
                    ...
                    bytes = BaseService2.SendPostRequest(url + sid.Value + "&lang=zh_CN&pass_ticket=" + LoginService2.Pass_Ticket, msg_json);
                    if (bytes == null)
                    {
                        continue;
                    }
        ...
        public string GetQun(string qunUserName)
        {
            ...
            if (sid != null && uin != null)
            {
                msg_json = "{\"BaseRequest\":{ \"Uin\":" + uin.Value+",\"Sid\":\""+sid.Value+"\",\"Skey\":\""+LoginService2.SKey+"\",\"DeviceID\":\"e"+s.ToString()+"\"},\"Count\":1,\"List\":[{\"UserName\":\""+qunUserName+"\",\"EncryChatRoomId\":\"\"}]}";
                ...
                foreach (string item in listCheckUrl)
                {
                    ...
                    bytes = BaseService2.SendPostRequest(url+"&r="+ DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalMilliseconds.ToString().Split(new char[] { '.' })[0] + "&pass_ticket=" + LoginService2.Pass_Ticket, msg_json);
                    string send_result = Encoding.UTF8.GetString(bytes);
